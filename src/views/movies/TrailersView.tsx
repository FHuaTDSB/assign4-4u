...
export const TrailersView = () => {
  ...
  const { data } =
    media == 'movie'
      ? useTmdb<TrailersResponse>(`${MOVIE_ENDPOINT}/${id}/videos`, {}, [id])
      : useTmdb<TrailersResponse>(`${TV_ENDPOINT}/${id}/videos`, {}, [id]);
  const trailerVideo =
    data?.results.find((video) => video.site === 'YouTube' && video.type === 'Trailer' && video.name?.toLowerCase().includes('official')) ||
